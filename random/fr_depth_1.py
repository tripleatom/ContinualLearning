sorting_analyzer = r"/Volumes/xieluanlabs/xl_cl/sortout/SNr1/SNr1_20260417/curated_analyzer"
...
contact_positions = probe.contact_positions  # (n_channels, 2)
...
probe_top = y_max + contact_size * 2
probe_bottom = y_min - contact_size * 2
tip_y = probe_bottom - shank_halfwidth * 1.2
...
fig, (ax, ax_fr, ax_tp) = plt.subplots(
    1, 3, figsize=(9, 12), sharey=True,
    gridspec_kw={"width_ratios": [3, 2, 2], "wspace": 0.08},
)
...
ax.set_ylim(probe_top + contact_size, tip_y - contact_size)  # invert: 0=top, deeper=down
ax.set_aspect("equal")
ax.axis("off")

# --- Firing rate vs depth side panel ---
unit_depths = np.array([contact_positions[best_channels[i]][1]
...
print(f"{'unit':>6}  {'depth(um)':>9}  {'rate(Hz)':>9}  {'t2p(ms)':>8}")
for i, uid in enumerate(unit_ids):
    print(f"{str(uid):>6}  {unit_depths[i]:9.1f}  "
          f"{firing_rates[i]:9.2f}  {trough_to_peak[i]:8.3f}")

plt.tight_layout()
plt.savefig("/Users/xiaorongzhang/Codes/ContinualLearning/random/probe_waveforms.png",
            dpi=200, bbox_inches="tight")
plt.show()
print("Done. Saved to probe_waveforms.png")
