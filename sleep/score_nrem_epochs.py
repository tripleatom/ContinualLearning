...
Runs once per active sleep session (pre / post), mirroring
compute_sleep_features.py's own `active_sleep_sessions(sleep_sessions)` loop:
compute_sleep_features.py writes one band-powers pickle PER SESSION
(`{session_name}{suffix}_all_shanks_band_powers.pkl`, suffix = '_pre'/'_post'),
not one for the whole day, so this script must read and score each session's
file separately too. It previously read a single bare
`{session_name}_all_shanks_band_powers.pkl` with no suffix -- a file that
compute_sleep_features.py has not written since band powers moved to the
per-session convention -- so it could never find its input for a two-session
day; SESSION_FILTER (both / 'pre' / 'post') is honored the same way every
other pipeline stage honors it.

Outputs (in low_freq/sleep_segmentation/), one pair per session:
  {session}{suffix}_sleep_periods.pkl   — nrem_mask, bouts, consolidated windows, scores
  {session}{suffix}_sleep_periods.png   — Buzsaki-style figure with colored state bar
"""
...
for session_key, session_cfg in sessions_to_run.items():
    ...
    session_low_freq_folder = bp_file.parent
    out_dir = resolve_output_folder(session_low_freq_folder / "sleep_segmentation")
    ...
    # ── save ────────────────────────────────────────────────────────────────────
    out_pkl = out_dir / f"{session_label}_sleep_periods.pkl"
    ...
    ax0.set_xticks([]); ax0.set_title(
        f"{session_label}  —  consolidated-sleep segmentation  "
    ...
    stamp = (f"Generated {datetime.now():%Y-%m-%d %H:%M:%S} by score_nrem_epochs.py | "
             f"session={session_label} shanks={list(use_shanks)} | source={bp_file} | "
             f"params={P}")
    fig.text(0.005, 0.001, stamp, fontsize=6, color="0.4", ha="left", va="bottom")

    out_png = out_dir / f"{session_label}_sleep_periods.png"
