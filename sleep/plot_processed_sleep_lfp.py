...
Reads the pkl written by plot_sleep_spectrograms.py (z-scored spectrogram + band
...
Differences from plot_sleep_spectrograms.py's own figure, per request:
  * NO seam dashed lines.
  * Band y-labels show the Hz range only (no band name).

Usage:
    python plot_processed_sleep_lfp.py [path_to_trace_data.pkl] [--out OUT.png]
"""
...
def parse_args():
    ...
    p.add_argument("pkl", nargs="?", default=DEFAULT_PKL,
                   help="path to a *_trace_data*.pkl exported by plot_sleep_spectrograms.py")
...
stamp = (
    f"Generated {datetime.now():%Y-%m-%d %H:%M:%S} by plot_processed_sleep_lfp.py  |  "
    ...
    f"kept={d['kept_duration_s']:.0f}s  |  reproduce: python plot_processed_sleep_lfp.py \"{pkl_path}\""
)
