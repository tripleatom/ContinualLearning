# -------------------------------------------------------------------------
# Presents pairs of static sinusoidal gratings on left/right. You can pass
# multiple orientation pairs (e.g., [(45,135), (0,90)]). For each pair, trials
# are balanced across sides and randomized. Each CSV row contains the same
# trial's stim_on_s and stim_off_s on a consistent timebase.
# -------------------------------------------------------------------------
...
# Global clock for on/off timestamps (SAME timebase for both)
...
# =========================
# 9) Build modular balanced trial list
# =========================
...
# =========================
# 9.5) Save per-run task metadata (for downstream tools, optional)
# =========================
...
np.savez_compressed(
    taskmeta_path,
    run_id=run_id,
    task_name="two_grating_passive_static",
    csv_path=log_path,
...
with open(log_path, "w", newline="") as f:
    ...
    for tr in trials:
        ...
        sync_patch.draw()
        win.flip()
        stim_off = global_clock.getTime()  # same timebase for ON/OFF
