"""
Compare multiple decoders for passive stim-type data across bin sizes (30-500 ms).
Data source : prepare_passive_stimtype.py  (passive_spikes_*.pkl)
Labels      : +1 stim-on class_pos  /  -1 stim-on class_neg  /  0 ITI
...
for bms in bin_sizes_ms:
    ...
    X, y, _, _ = prepare_passive_stim_type(
...
plot_panel(ax_top, results, bin_sizes_ms, 'overall',
           f'Overall accuracy - stim type ({_fmt_class(class_pos)} vs {_fmt_class(class_neg)} vs ITI)', 0.0)
ax_top.axhline(mean_chance, color='gray', linestyle='--', linewidth=1.2,
               label=f'Chance ({mean_chance:.2f})')
...
for ax, c in zip(axes_bot, all_classes):
    ...
    ax.axhline(mean_chance, color='gray', linestyle='--', linewidth=1.2,
               label=f'Chance ({mean_chance:.2f})')
...
stem    = f"stimtype_{session}_pos-{_stem_part(class_pos)}_neg-{_stem_part(class_neg)}_{n_splits}fold"
...
X_bar, y_bar, _, _ = prepare_passive_stim_type(
    pkl_file, class_pos, class_neg, PASSIVE_COL_MAP,
    bin_size_sec=bar_bin_ms / 1000.0,
    random_state=random_state,
)
...
fig2.suptitle(
    f'Per-classifier decoding - stim type  |  bin = {bar_bin_ms} ms, {n_splits}-fold CV\n'
    f'+1: {_fmt_class(class_pos)}  vs  -1: {_fmt_class(class_neg)}  vs ITI  |  '
