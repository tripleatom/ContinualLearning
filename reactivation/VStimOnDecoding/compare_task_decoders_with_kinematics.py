"""
All-kinematic-features twin of the velocity-as-feature and heading-as-feature
scripts. Adds five behavioral columns to the classifier input and compares
against the firing-rate-only baseline:

  features added = [
      speed_cms,                 # linear, cm/s
      sin(heading),  cos(heading),     # circular, 0–360° encoded as (sin, cos)
      sin(head_angle), cos(head_angle) # circular, radians encoded as (sin, cos)
  ]

Aggregation per task bin: mean over all clean DLC samples falling inside the
bin. For circular variables this is the magnitude-and-direction circular mean
(equivalent to mean of sin and mean of cos, no unwrapping needed).

For each bin size:
  baseline  : X = firing rates                                      (n_bins, n_units)
  augmented : X = [firing rates | speed | sin_h | cos_h | sin_ha | cos_ha]
                                                                    (n_bins, n_units + 5)
...
Outputs (under <pkl_folder>/reactivation/):
  task_<session>_kinematics_feature_vs_baseline_<n>fold.png
  task_<session>_kinematics_feature_delta_<n>fold.png
  task_<session>_kinematics_feature_bar<bms>ms.png
  task_<session>_kinematics_feature_<n>fold.pkl
"""
...
# ------------------------------------------------------------------ #
#  Step 2 — sweep bin sizes: baseline (X) vs augmented ([X | kin])    #
# ------------------------------------------------------------------ #
...
for bms_idx, bms in enumerate(bin_sizes_ms):
    ...
    extras, keep = build_task_kinematics(bc, bms / 1000.0, kin_samples)
    n_dropped = int((~keep).sum())
    X = X[keep]; y = y[keep]
    extras = extras[keep].astype(X.dtype)
    ...
    print(f"  n_total_bins={len(y)}  n_dropped_no_tracking={n_dropped}  "
          ...
          f"(added: speed, sin/cos heading, sin/cos head_angle)")
...
def _plot_overlay(ax, results_a, results_b, bin_sizes_ms, key, title):
    """Solid = augmented (firing rates + kinematics), dashed = baseline (firing rates only)."""
    markers = ['o', 's', '^', 'D', 'v', 'P', '*']
...
_plot_overlay(ax_top, results_augmented, results_baseline, bin_sizes_ms,
              'overall',
              'Overall accuracy — solid: firing rates + speed + sin/cos(heading + head_angle), dashed: firing rates only')
_style_handles = [
    plt.Line2D([], [], color='black', linestyle='-',  linewidth=1.6, label='+ kinematics'),
...
ax_top.legend(clf_handles + _style_handles,
              clf_labels  + ['+ kinematics', 'baseline'],
...
info = (
    ...
    f"Features added: speed, sin/cos(heading), sin/cos(head_angle)  |  "
...
stem = f"task_{session}_kinematics_feature_vs_baseline_{n_splits}fold"
...
# ------------------------------------------------------------------ #
#  Figure: kinematics-feature gain (Δ ± 95% CI) per classifier        #
# ------------------------------------------------------------------ #
...
_plot_delta(ax_d_top, delta_summary,
            'Overall Δ accuracy: gain from adding speed + sin/cos(heading + head_angle) (paired, 95% CI bootstrap)')
...
plt.tight_layout()
fig_d_path = out_dir / f"task_{session}_kinematics_feature_delta_{n_splits}fold.png"
...
extras_bar, keep_bar = build_task_kinematics(bc_bar, bar_bin_ms / 1000.0, kin_samples)
X_bar, y_bar = X_bar[keep_bar], y_bar[keep_bar]
extras_bar = extras_bar[keep_bar].astype(X_bar.dtype)
X_bar_aug = np.hstack([X_bar, extras_bar])
...
def _bar_three(ax, base_means, base_stds, aug_means, aug_stds,
    ...
    bars_a = ax.bar(x, aug_means, bar_w, color=color_aug,
                    yerr=aug_stds, capsize=3, error_kw=dict(linewidth=1), label='+ Kinematics')
...
_bar_three(
    ...
    f'Overall accuracy — baseline vs + kinematics vs shuffle  (bin = {bar_bin_ms} ms)',
    'Accuracy',
)
...
fig2.suptitle(
    f'Per-classifier decoding — kinematics features vs firing-rate baseline  |  '
    f'bin = {bar_bin_ms} ms, {n_splits}-fold CV\n'
    f'Balanced train / natural-ratio test',
    fontsize=11, y=1.01,
)
plt.tight_layout()
fig2_path = out_dir / f"task_{session}_kinematics_feature_bar{bar_bin_ms}ms.png"
...
print("Interpretation: Δ > 0 and CI excluding 0 ⇒ adding speed + heading + head_angle as features "
...
# ------------------------------------------------------------------ #
pkl_out = out_dir / f"task_{session}_kinematics_feature_{n_splits}fold.pkl"
