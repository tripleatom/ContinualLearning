"""
Velocity-as-feature twin of compare_task_reward_decoders_velocity_matched.py.

Instead of controlling for running speed by sub-sampling ITI to match the
stim-on speed histogram, this script *adds* per-bin speed as an extra input
feature to the classifier and compares against the firing-rate-only baseline.

For each bin size:
  baseline  : X = firing rates                 shape (n_bins, n_units)
  augmented : X = [firing rates | speed_cms]   shape (n_bins, n_units + 1)
...
as DiscriminationTask/grating/plot_trial_traces.py.

Outputs (under <pkl_folder>/reactivation/):
  task_<session>_velocity_feature_vs_baseline_<n>fold.png
  task_<session>_velocity_feature_delta_<n>fold.png
  task_<session>_velocity_feature_bar<bms>ms.png
  task_<session>_velocity_feature_<n>fold.pkl
"""
...
# ------------------------------------------------------------------ #
#  Step 1 — load DLC, build kinematic sample arrays (window frame)    #
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
#  Step 2 — sweep bin sizes: baseline (X) vs augmented ([X | speed])  #
# ------------------------------------------------------------------ #
...
for bms_idx, bms in enumerate(bin_sizes_ms):
    ...
    extras, keep = build_task_kinematics(bc, bms / 1000.0, kin_samples, columns=SPEED_ONLY_COLUMNS)
    ...
    print(f"  n_total_bins={len(y)}  n_dropped_no_tracking={n_dropped}  "
          ...
          f"X_aug feature dim={X_aug.shape[1]} (added: speed_cms)")

    pbar = tqdm(total=n_repeats * len(CLASSIFIERS),
                desc=f"  bin {bms} ms CV", ncols=90, leave=True)
    for r in range(n_repeats):
        # Vary CV seed across repeats so each repeat gives an independent
        # paired estimate (same seed used by baseline and augmented inside).
    ...
    if bins_done < n_bins_sweep:
        # Remaining bins are smaller -> faster. Project ETA assuming cost ∝ 1/bin_ms.
...
def _plot_overlay(ax, results_a, results_b, bin_sizes_ms, key, title):
    """Solid = augmented (firing rates + speed), dashed = baseline (firing rates only)."""
    markers = ['o', 's', '^', 'D', 'v', 'P', '*']
...
_plot_overlay(ax_top, results_augmented, results_baseline, bin_sizes_ms,
              'overall',
              'Overall accuracy — solid: firing rates + speed, dashed: firing rates only')
_style_handles = [
    plt.Line2D([], [], color='black', linestyle='-',  linewidth=1.6, label='+ speed'),
...
info = (
    ...
    f"Feature: + 1 column of mean per-bin speed (cm/s)  |  "
...
stem = f"task_{session}_velocity_feature_vs_baseline_{n_splits}fold"
...
# ------------------------------------------------------------------ #
#  Figure: velocity-feature gain (Δ ± 95% CI) per classifier          #
# ------------------------------------------------------------------ #
...
_plot_delta(ax_d_top, delta_summary,
            'Overall Δ accuracy: gain from adding speed as feature (paired, 95% CI bootstrap)')
...
info_d = (
    ...
    f"Δ > 0 ⇒ speed feature helped; CI excluding 0 ⇒ effect is significant"
)
fig_d.text(0.5, -0.01, info_d, ha='center', va='top', fontsize=8, color='gray')
plt.tight_layout()
fig_d_path = out_dir / f"task_{session}_velocity_feature_delta_{n_splits}fold.png"
...
extras_bar, keep_bar = build_task_kinematics(bc_bar, bar_bin_ms / 1000.0, kin_samples, columns=SPEED_ONLY_COLUMNS)
...
def _bar_three(ax, base_means, base_stds, aug_means, aug_stds,
    ...
    bars_a = ax.bar(x, aug_means, bar_w, color=color_aug,
                    yerr=aug_stds, capsize=3, error_kw=dict(linewidth=1), label='+ Speed')
...
_bar_three(
    ...
    f'Overall accuracy — baseline vs + speed vs shuffle  (bin = {bar_bin_ms} ms)',
    'Accuracy',
)
...
fig2.suptitle(
    f'Per-classifier decoding — speed as feature vs firing-rate baseline  |  '
    f'bin = {bar_bin_ms} ms, {n_splits}-fold CV\n'
    f'Balanced train / natural-ratio test',
    fontsize=11, y=1.01,
)
plt.tight_layout()
fig2_path = out_dir / f"task_{session}_velocity_feature_bar{bar_bin_ms}ms.png"
...
print("Interpretation: Δ > 0 and CI excluding 0 ⇒ adding speed as a feature "
...
# ------------------------------------------------------------------ #
pkl_out = out_dir / f"task_{session}_velocity_feature_{n_splits}fold.pkl"
with open(pkl_out, 'wb') as f:
    pickle.dump({
        ...
        'feature_columns_added':   SPEED_ONLY_COLUMNS,
        'n_repeats':               n_repeats,
