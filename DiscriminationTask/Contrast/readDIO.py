...
def get_white_on_left(filepath):
    """
    Extract white on left values for all trials in a session
    ...
    Example:
    --------
    >>> white_positions = get_white_on_left('session_data.json')
    >>> print(f"Total trials: {len(white_positions)}")
    >>> print(f"First 10 trials: {white_positions[:10]}")
    """
    with open(filepath, 'r') as f:
        data = json.load(f)

    trials = data.get('trials', [])
    white_on_left = [trial.get('whiteOnLeft', None) for trial in trials]

    return white_on_left

def process_behavior_trial_responses(rec_folder, trial_start, trial_end, white_on_left, overwrite=True):
    """
    ...
    The function:
      ...
      - Stores trial information (white_on_left condition)
    ...
    Parameters:
      ...
      trial_start (np.ndarray): Array of trial start indices (in samples).
      trial_end (np.ndarray): Array of trial end indices (in samples).
      white_on_left (np.ndarray): Boolean array indicating white stimulus on left side.
    ...
    white_on_left = np.array(white_on_left, dtype=bool)
    ...
    print(f"White on left shape: {white_on_left.shape}")
    
    # Verify array lengths match
    assert len(trial_start) == len(trial_end) == len(white_on_left), \
        "trial_start, trial_end, and white_on_left must have the same length"
    
    # Unique stimulus conditions
    unique_conditions = np.unique(white_on_left)
    n_conditions = len(unique_conditions)
    print(f"Unique conditions (white_on_left): {unique_conditions}")
    
    # Compute the number of repeats per condition
    n_left = np.sum(white_on_left)
    n_right = np.sum(~white_on_left)
    print(f"Trials with white on left: {n_left}")
    print(f"Trials with white on right: {n_right}")
    ...
    session_folder = project_folder / f"sortout/{animal_id}/{animal_id}_{session_id}"
        ...
        for sorting_results_folder in sorting_results_folders:
            ...
            for i, unit_id in enumerate(unit_ids):
                ...
                for trial_idx in range(n_trials):
                    ...
                    trial_white_left = white_on_left[trial_idx]
                    
                    # Find condition index
                    condition_idx = 1 if trial_white_left else 0
                    
                    # Calculate repeat number for this condition
                    condition_trials = []
                    for prev_trial in range(trial_idx):
                        if white_on_left[prev_trial] == trial_white_left:
                    ...
                    trial_info = {
                        ...
                        'white_on_left': bool(trial_white_left),
    ...
    *,
    ...
    for trial_idx in range(len(trial_start)):
        trial_params = {
            ...
            'white_on_left': bool(white_on_left[trial_idx]),
    ...
    neural_data = {
        ...
        'experiment_parameters': {
            ...
            'n_conditions': len(unique_conditions),
            'n_left_trials': int(np.sum(white_on_left)),
            'n_right_trials': int(np.sum(~white_on_left)),
        },
        'trial_info': {
            'white_on_left': white_on_left.tolist(),
...
if __name__ == '__main__':
    ...
    rec_folder  = r"D:\cl\ephys\CnL39SG_20260107_163640.rec"
    ...
    task_file = r"D:\cl\SphereTask\sphereTaskBuild\BehaviorData\CnL39_2026-01-07_Session001_Data.json"

    white_on_left = get_white_on_left(task_file)
    ...
    # white_on_left = np.random.rand(n_trials) > 0.5  # Random left/right
    n_DIO = len(trial_start)
    n_trials = len(white_on_left)

    if n_DIO != n_trials:
        Warning(f"The number of trials from DIO ({n_DIO}) does not match the number of trials from behavior file ({n_trials}). Please check your data.")
