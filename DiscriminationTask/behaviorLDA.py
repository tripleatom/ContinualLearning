...
def perform_lda_analysis(firing_rates, condition_labels):
    ...
    # Fit full model for coefficients only (not used for performance evaluation)
...
def _save_individual_subfigures(results, unit_ids, trial_info, base_save_path):
    ...
    try:
        ...
        plt.tight_layout()
        fig.savefig(lda_folder / "3_cv_scores.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: 3_cv_scores.png")
    except Exception as e:
        print(f"  ✗ Failed: 3_cv_scores - {e}")
    ...
    try:
        if hasattr(results['lda_full'], 'coef_'):
            ...
            ax.grid(True, alpha=0.3, axis='x')
            plt.tight_layout()
            fig.savefig(lda_folder / "4_lda_coefficients.png", dpi=150, bbox_inches='tight')
            plt.close(fig)
            print(f"  ✓ Saved: 4_lda_coefficients.png")
    except Exception as e:
        print(f"  ✗ Failed: 4_lda_coefficients - {e}")
    ...
    try:
        ...
        plt.setp(ax.get_xticklabels(), rotation=45, ha='right')
        plt.tight_layout()
        fig.savefig(lda_folder / "5_trial_distribution.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: 5_trial_distribution.png")
    except Exception as e:
        print(f"  ✗ Failed: 5_trial_distribution - {e}")
    ...
    • Right trials: {trial_info['n_trials_per_condition'][0]}
        ...
        plt.tight_layout()
        fig.savefig(lda_folder / "6_summary_text.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: 6_summary_text.png")
    except Exception as e:
        print(f"  ✗ Failed: 6_summary_text - {e}")
    ...
    try:
        if hasattr(results['lda_full'], 'coef_'):
            ...
            ax.grid(True, alpha=0.3, axis='x')
            plt.tight_layout()
            fig.savefig(lda_folder / "7_feature_importance.png", dpi=150, bbox_inches='tight')
            plt.close(fig)
            print(f"  ✓ Saved: 7_feature_importance.png")
    except Exception as e:
        print(f"  ✗ Failed: 7_feature_importance - {e}")
    ...
    try:
        ...
        plt.setp(ax.get_xticklabels(), rotation=90, fontsize=8)
        plt.tight_layout()
        fig.savefig(lda_folder / "8_firing_rate_comparison.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: 8_firing_rate_comparison.png")
    except Exception as e:
        print(f"  ✗ Failed: 8_firing_rate_comparison - {e}")
...
def run_analysis(data_path, time_window=(0.0, None), save_plots=True, 
    ...
    if output_path is None and save_plots:
        ...
        output_path = session_dir / "behavior_analysis" / (Path(data_path).stem + ".lda_analysis.png")
...
if __name__ == "__main__":
    ...
    try:
        results = run_analysis(
            data_path=DATA_PATH,
            time_window=(0.0, 1.0),  # Analyze first 1 second of each trial
            save_plots=True
        )
