"""
Logistic Regression Analysis for Behavior Trial Neural Data
...
def perform_logreg_analysis(firing_rates, condition_labels, C=1.0):
    """
    Perform logistic regression analysis with cross-validation for binary classification.

    Returns:
    --------
    dict : Dictionary with logistic regression results including:
    ...
    print(f"\nLogistic Regression Analysis (C={C}):")
    ...
    clf_cv = LogisticRegression(C=C, max_iter=1000, solver='lbfgs')
...
def create_analysis_figure(results, unit_ids, trial_info, save_path=None):
    """Create comprehensive logistic regression analysis visualization."""
    ...
    _plot_logreg_weights(fig, results, unit_ids)
...
def _plot_decision_score_projection(fig, data, labels, condition_names, colors):
    ...
    ax.set_xlabel('Decision Score (Log-Odds)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Condition', fontsize=12, fontweight='bold')
    ax.set_yticks([0, 1])
    ax.set_yticklabels(list(condition_names.values()))
    ax.set_title('LogReg Decision Score Projection', fontsize=14, fontweight='bold')
...
def _plot_decision_score_histogram(fig, data, labels, condition_names, colors):
    ...
    ax.set_title('LogReg Decision Score Distribution', fontsize=14, fontweight='bold')
    ax.legend(fontsize=10)
...
def _plot_logreg_weights(fig, results, unit_ids):
...
def _save_individual_subfigures(results, unit_ids, trial_info, base_save_path):
    ...
    out_folder = resolve_output_folder(base_path.parent)
    ...
    try:
        ...
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        fig.savefig(out_folder / "logreg_1_decision_projection.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: logreg_1_decision_projection.png")
    except Exception as e:
        print(f"  ✗ Failed: logreg_1_decision_projection - {e}")
    ...
    try:
        ...
        ax.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
        fig.savefig(out_folder / "logreg_2_decision_histogram.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: logreg_2_decision_histogram.png")
    except Exception as e:
        print(f"  ✗ Failed: logreg_2_decision_histogram - {e}")
    ...
    try:
        ...
        plt.tight_layout()
        fig.savefig(out_folder / "logreg_3_cv_scores.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: logreg_3_cv_scores.png")
    except Exception as e:
        print(f"  ✗ Failed: logreg_3_cv_scores - {e}")
    ...
    try:
        if hasattr(results['model'], 'coef_'):
            ...
            ax.set(yticks=range(len(sorted_idx)), yticklabels=[unit_ids[i] for i in sorted_idx],
                   xlabel='LogReg Weight', title='Top Discriminative Units (Positive=Left, Negative=Right)')
            ax.invert_yaxis()
            ax.axvline(x=0, color='black', linestyle='-', linewidth=1)
            ax.grid(True, alpha=0.3, axis='x')
            plt.tight_layout()
            fig.savefig(out_folder / "logreg_4_weights.png", dpi=150, bbox_inches='tight')
            plt.close(fig)
            print(f"  ✓ Saved: logreg_4_weights.png")
    except Exception as e:
        print(f"  ✗ Failed: logreg_4_weights - {e}")
    ...
    try:
        ...
        plt.setp(ax.get_xticklabels(), rotation=45, ha='right')
        plt.tight_layout()
        fig.savefig(out_folder / "logreg_5_trial_distribution.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: logreg_5_trial_distribution.png")
    except Exception as e:
        print(f"  ✗ Failed: logreg_5_trial_distribution - {e}")
    ...
    • Right trials: {trial_info['n_trials_per_condition'][0]}
        ...
        plt.tight_layout()
        fig.savefig(out_folder / "logreg_6_summary_text.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: logreg_6_summary_text.png")
    except Exception as e:
        print(f"  ✗ Failed: logreg_6_summary_text - {e}")
    ...
    try:
        if hasattr(results['model'], 'coef_'):
            ...
            ax.set(yticks=range(len(top_idx)), yticklabels=[unit_ids[i] for i in top_idx],
                   xlabel='|LogReg Weight|', title='Top Discriminative Units')
            ax.invert_yaxis()
            ax.grid(True, alpha=0.3, axis='x')
            plt.tight_layout()
            fig.savefig(out_folder / "logreg_7_feature_importance.png", dpi=150, bbox_inches='tight')
            plt.close(fig)
            print(f"  ✓ Saved: logreg_7_feature_importance.png")
    except Exception as e:
        print(f"  ✗ Failed: logreg_7_feature_importance - {e}")
    ...
    try:
        ...
        plt.setp(ax.get_xticklabels(), rotation=90, fontsize=8)
        plt.tight_layout()
        fig.savefig(out_folder / "logreg_8_firing_rate_comparison.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  ✓ Saved: logreg_8_firing_rate_comparison.png")
    except Exception as e:
        print(f"  ✗ Failed: logreg_8_firing_rate_comparison - {e}")
...
def run_analysis(data_path, time_window=(0.0, None), C=1.0, save_plots=True,
    ...
    Args:
        ...
        C: Logistic regression regularization parameter (smaller = more regularization)
        save_plots: Whether to save the figure
        output_path: Custom output path

    Returns:
        Tuple of (logreg_results, firing_rates, condition_labels, unit_ids, selectivity)
    """
    ...
    logreg_results = perform_logreg_analysis(firing_rates, condition_labels, C=C)
    ...
    if output_path is None and save_plots:
        ...
        output_path = session_dir / "behavior_analysis" / (Path(data_path).stem + ".logreg_analysis.png")

    fig = create_analysis_figure(
        logreg_results, unit_ids, trial_info,
        save_path=output_path if save_plots else None
    )

    return logreg_results, firing_rates, condition_labels, unit_ids, selectivity
...
if __name__ == "__main__":
    ...
    try:
        ...
        logreg_results, firing_rates, condition_labels, unit_ids, selectivity = results

        print("\n" + "="*60)
        print("Analysis complete!")
        print("="*60)
        print(f"\nKey Results:")
        print(f"  Classification accuracy: {logreg_results['cv_scores'].mean():.3f}")
        print(f"  Above chance: {logreg_results['cv_scores'].mean() - 0.5:+.3f}")
