...
def initiate_nwb(rec_file, nwb_path, metadata=None, ishank=0):
    ...
    geom_path = os.path.join(script_dir, "geom.csv")
    shutil.copy2(params_path, mda_folder)
    shutil.copy2(geom_path, mda_folder)

    recording = se.read_mda_recording(mda_folder, mda_file, params_fname="params.json",
                                      geom_fname="geom.csv")
...
def append_nwb(nwb_path, rec_file, ishank=0, metadata=None):
    ...
    rec_ephys = se.read_mda_recording(
        mda_folder, mda_file).get_traces(channel_ids=channel_index)
    _append_nwb_dset(
