...
def visualize_lda_decoding_chosen_angles(npz_file, chosen_angles=[45, 135], cv_folds=5, random_state=42):
    """
    Visualizes LDA decoding results for only the chosen angles (e.g., 45° and 135°).
    This function performs LDA discrimination and creates three figures:
    ...
    Parameters:
      ...
      chosen_angles (list): List of angles (in degrees) to discriminate, e.g., [45, 135].
    ...
    # Create label vector y for grating orientations
    y = np.repeat(unique_orientation, n_phase * n_sf * n_repeats)
    y = np.array(y)

    # Filter for the chosen angles only
    mask = np.isin(y, chosen_angles)
    X = X[mask, :]
    y = y[mask]
    print(f"Trials after filtering for chosen angles {chosen_angles}: {X.shape[0]}")

    # Encode labels (convert angles to discrete classes)
    le = LabelEncoder()
    ...
    # Fit LDA on the entire dataset for projection
    clf.fit(X, y_class)
    ...
    if X_lda.shape[1] >= 2:
        ...
        for i, ori_enc in enumerate(unique_orientations_encoded):
            ...
            # Convert encoded orientation back to original angle for labeling
    ...
    fig.suptitle(f"LDA Decoding for Animal: {animal_id}, Session: {session_id}\nChosen angles: {chosen_angles}", fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])

    # Save Figure 1
    embedding_folder = session_folder / "embedding"
    embedding_folder.mkdir(parents=True, exist_ok=True)
    save_path = embedding_folder / "LDA_decoding_chosen_angles.png"
    ...
    # Get cross-validated predictions for each trial
    ...
    for ori_enc in unique_orientations_encoded:
        ...
        # Convert encoded label back to the original angle
        ori_val = le.inverse_transform([ori_enc])[0]
    ...
    save_path2 = embedding_folder / "LDA_per_orientation_accuracy_chosen_angles.png"
    ...
    save_path3 = embedding_folder / "LDA_real_vs_shuffled_accuracy_chosen_angles.png"
...
if __name__ == '__main__':
    npz_file_path = '/Volumes/xieluanlabs/xl_cl/code/sortout/CnL22/250406_002515/static_grating_responses.npz'
